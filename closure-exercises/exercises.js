...
function once(func) {
	...
	function returningFunction(input) {
		if (computedResult === undefined) {
			computedResult = func(input);
...
function saveOutput(func, magicWord) {
	...
	function returningFunction(input) {
		...
		if (
			Object.keys(computedOutputs).findIndex((element) => element == input) >= 0
		) {
			return computedOutputs[input];
		} else {
			computedOutputs[input] = runningFunction(input);
			return computedOutputs[input];
		}
	}
